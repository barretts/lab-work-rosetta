...
class RosettaCLI:
    ...
    def stats(self) -> Dict[str, int]:
        ...
        tables = [
            'loinc_concept', 'lis_mapping', 'vocabulary_mapping',
            'reference_distribution', 'nci_concept', 'concept_description',
            'severity_rule', 'source_system'
        ]
...
Examples:
  ...
  rosetta stats
    ...
    # translate command
    p_trans = subparsers.add_parser('translate', aliases=['t'], help='Translate LIS shorthand to LOINC')
    ...
    try:
        if args.command in ('translate', 't'):
            ...
            if args.json:
                ...
                print("=" * 60)
                cols = ['sex', 'age_min_years', 'age_max_years', 'ref_low', 'ref_high', 'unit_of_measure', 'n_samples']
            ...
            if args.json:
                ...
                print("\nCritical Values")
                print("=" * 60)
                cols = ['loinc_code', 'long_common_name', 'direction', 'threshold_operator', 'threshold_value', 'unit_of_measure']
                print(format_table(results, cols))
            else:
