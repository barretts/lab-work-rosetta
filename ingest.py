...
NHANES_CBC_MAP = {
    ...
    'LBXMC': {'loinc': '786-4', 'name': 'Mean cell hemoglobin concentration (MCHC)', 'unit': 'g/dL'},
...
class RosettaIngestor:
    ...
    def _create_population(self, name, desc, size, start, end, region, url) -> int:
        ...
        cursor = self.conn.execute(
            "SELECT population_id FROM reference_population WHERE dataset_name = ?",
            (name,)
        )
        row = cursor.fetchone()
        if row:
    ...
    def _process_nhanes_file(self, filepath: Path, demo_df: pd.DataFrame, var_map: dict, pop_id: int):
            ...
            for sex in ['M', 'F', 'A']:  # A = All
                for age_min, age_max in age_bins:
                    ...
                    self._insert_distribution(
                        meta['loinc'], pop_id, sex if sex != 'A' else None,
                        age_min, age_max, stats, meta.get('unit')
                    )
    ...
    def _insert_distribution(self, loinc: str, pop_id: int, sex: str, 
                            age_min: float, age_max: float, stats: dict, unit: str):
        ...
        try:
            self.conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                loinc, pop_id, sex, age_min, age_max,
                stats['n'], stats['mean'], stats['std'],
                stats['p01'], stats['p025'], stats['p05'], stats['p25'],
                stats['p50'], stats['p75'], stats['p95'], stats['p975'], stats['p99'],
                stats['p025'], stats['p975'],  # Use 2.5-97.5 percentiles as ref range
                unit
            ))
        except Exception as e:
    ...
    def ingest_nci_thesaurus(self):
        ...
        lab_types = [
            ...
            'Diagnostic Procedure', 'Clinical Attribute', 'Finding',
            'Sign or Symptom', 'Pharmacologic Substance', 'Enzyme',
            'Amino Acid, Peptide, or Protein', 'Biologically Active Substance'
        ]
        
        columns = [
            'code', 'concept_iri', 'parents', 'synonyms', 'definition',
            'display_name', 'concept_status', 'semantic_type', 'concept_in_subset'
        ]
        ...
        for chunk in pd.read_csv(
            ...
            for _, row in chunk.iterrows():
                ...
                code = str(row['code'])
                preferred = str(row.get('synonyms', '')).split('|')[0] if row.get('synonyms') else ''
    ...
    def ingest_ctcae(self):
        ...
        for sheet in xlsx.sheet_names:
            if sheet.lower() in ['title page', 'instructions', 'appendix', 'toc', 'table of contents']:
                continue
            ...
            try:
                ...
                for idx in range(header_row + 1, len(df)):
                    ...
                    for grade, col_idx in grade_cols.items():
                        ...
                        if definition and definition != 'nan' and definition != '-' and len(definition) > 1:
                            try:
                                self.conn.execute("""
        ...
        for _, row in df.iterrows():
            ...
            try:
                self.conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?)
                """, (system_id, str(shorthand), str(name) if name else None, str(loinc), 'seed_data'))
                count += 1
            except Exception as e:
