...
def fetch_loinc_snomed_mapping(loinc_code: str, tgt_url: str) -> dict:
    ...
    try:
        ...
        if cui:
            ...
            atoms_params = {"sabs": "SNOMEDCT_US", "ticket": ticket2}
            ...
            if atoms_response.status_code == 200:
                ...
                result = {
                    ...
                    "cui": cui,
                    "snomed_codes": [
                        {"code": a.get("code"), "name": a.get("name"), "tty": a.get("termType")}
                        for a in snomed_atoms
                        if a.get("code")
                    ],
                }
...
def fetch_rxnorm_interactions(drug_name: str) -> dict:
    ...
    try:
        ...
        props = (
            props_response.json().get("properties", {}) if props_response.status_code == 200 else {}
        )
                        ...
                        interactions.append(
                            {
                                ...
                                "drug": pair.get("interactionConcept", [{}])[1]
                                .get("minConceptItem", {})
                                .get("name"),
                            }
                        )
    ...
    try:
        for i, loinc_code in enumerate(to_process):
            ...
            if result.get("snomed_codes"):
                ...
                if i % 20 == 0:
                    logger.info(
                        f"[{i+1}/{len(to_process)}] ✓ {loinc_code}: {len(result['snomed_codes'])} SNOMED codes"
                    )
            else:
                no_mapping += 1
                if i % 100 == 0:
...
def fetch_loinc_snomed_parallel(to_process: list, progress: dict, workers: int):
    ...
    async def fetch_single(
        loinc_code: str, semaphore: asyncio.Semaphore, executor: ThreadPoolExecutor
    ):
        """Fetch a single code with semaphore control."""
        async with semaphore:
            loop = asyncio.get_event_loop()
            result = await loop.run_in_executor(
                executor, fetch_loinc_snomed_mapping, loinc_code, tgt_url
            )
        ...
        try:
            with ThreadPoolExecutor(max_workers=workers) as executor:
                for batch_start in range(0, len(to_process), BATCH_SIZE):
                    ...
                    tasks = [fetch_single(code, semaphore, executor) for code in batch]
...
def fetch_common_drug_interactions():
    ...
    for i, drug in enumerate(drugs):
        ...
        if result.get("rxcui"):
            logger.info(
                f"[{i+1}/{len(drugs)}] ✓ {drug}: RxCUI {result['rxcui']}, {len(result.get('interactions', []))} interactions"
            )
        else:
...
def status():
    ...
    # Count cache files
    snomed_cached = (
        len(list((CACHE_DIR / "loinc_snomed").glob("*.json")))
        if (CACHE_DIR / "loinc_snomed").exists()
        else 0
    )
    rxnorm_cached = (
        len(list((CACHE_DIR / "rxnorm").glob("*.json"))) if (CACHE_DIR / "rxnorm").exists() else 0
    )
