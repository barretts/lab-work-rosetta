...
def ingest_loinc_core(conn: sqlite3.Connection, loinc_path: Path = LOINC_PATH):
    ...
    lab_df = df[
        (df['STATUS'] == 'ACTIVE') & 
        (df['CLASS'].isin(LAB_CLASSES) | df['CLASS'].str.contains('|'.join(LAB_CLASSES), na=False))
    ]
        ...
        if pd.notna(shortname) and shortname:
            ...
            try:
                conn.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?)
                ''', (sys_id, str(shortname), str(longname)[:200] if pd.notna(longname) else None, 
                      loinc, 'loinc_shortname'))
                map_count += 1
            except Exception:
                pass
