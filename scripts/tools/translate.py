...
def translate_single(test_name: str):
    ...
    if r:
        ...
        if enrichment["reference_low"]:
            print(
                f"  Reference: {enrichment['reference_low']}-{enrichment['reference_high']} {enrichment['unit']}"
            )

        if enrichment["critical_low"] or enrichment["critical_high"]:
            crit = []
            if enrichment["critical_low"]:
                crit.append(f"Low {enrichment['critical_low']}")
            if enrichment["critical_high"]:
                crit.append(f"High {enrichment['critical_high']}")
...
Examples:
  ...
  python translate.py --single "HbA1c"
    ...
    parser.add_argument(
        "-c", "--confidence", type=float, default=0.5, help="Minimum confidence (0-1)"
    )
