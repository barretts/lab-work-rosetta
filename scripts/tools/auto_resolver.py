...
class AutoResolver:
    ...
    def batch_resolve(
        self, texts: List[str], min_confidence: float = 0.5
    ) -> Dict[str, Optional[Resolution]]:
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        print(
            f"Database: {stats['known_mappings']} known, {stats['learned_mappings']} learned, {stats['loinc_codes']} LOINC codes"
        )
